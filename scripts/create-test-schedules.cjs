...
async function main() {
  ...
  if (!mockTokenAddress || !distributionAddress) {
    ...
    console.log("Please ensure VITE_TOKEN_CONTRACT_ADDRESS and VITE_DISTRIBUTION_CONTRACT_ADDRESS are set");
    return;
  }

  // Get contract instances
  const mockToken = await hre.ethers.getContractAt("MockERC20", mockTokenAddress);
  const distribution = await hre.ethers.getContractAt("CharityScheduledDistribution", distributionAddress);
  ...
  const approveTx1 = await mockToken.connect(donor1).approve(distributionAddress, schedule1Amount);
  await approveTx1.wait();
  console.log("✅ Approved token transfer");

  // Create the schedule
  const scheduleTx1 = await distribution.connect(donor1).createSchedule(
    testCharities[0],
    mockTokenAddress,
    schedule1Amount
  );
  await scheduleTx1.wait();
  console.log(`✅ Created schedule: 120 tokens over 12 months to charity ${testCharities[0]}`);
  ...
  // First approve the tokens
  const approveTx2 = await mockToken.connect(donor2).approve(distributionAddress, schedule2Amount);
  await approveTx2.wait();
  console.log("✅ Approved token transfer");

  // Create the schedule
  const scheduleTx2 = await distribution.connect(donor2).createSchedule(
    testCharities[1],
    mockTokenAddress,
    schedule2Amount
  );
  await scheduleTx2.wait();
  console.log(`✅ Created schedule: 240 tokens over 12 months to charity ${testCharities[1]}`);

  console.log("\n✨ Test scheduled donations created successfully!");
  console.log("\nYou can now:");
  console.log("1. Log in as a donor account to see the scheduled donations");
  console.log(`2. Use these test donor addresses: ${donor1.address} or ${donor2.address}`);
