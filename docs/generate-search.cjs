...
function findMarkdownFiles(dir, fileList = []) {
  ...
  files.forEach(file => {
    ...
    } else if (file.endsWith('.md') && !file.startsWith('test-') && !file.includes('TEST_REPORT') && !file.includes('README-SEARCH') && !file.includes('DEPLOYMENT-CHECKLIST') && !file.includes('SEARCH-ANALYTICS')) {
...
function generateSearchData() {
  ...
  markdownFiles.forEach(filePath => {
    try {
      ...
      let url = path.relative(docsDir, filePath)
        .replace(/index\.md$/, '')
        .replace(/\.md$/, '/')
        .replace(/\\/g, '/');
