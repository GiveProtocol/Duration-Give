...
import type { MockComponentTemplate, MockDataTemplate, MockOverridesTemplate } from '../templates';
...
describe('templates', () => {
  ...
  describe('MOCK_TEMPLATE_EXAMPLES', () => {
    it('contains example code string', () => {
      expect(MOCK_TEMPLATE_EXAMPLES).toContain('Example 1: Creating a typed mock component');
      expect(MOCK_TEMPLATE_EXAMPLES).toContain('Example 2: Creating typed mock data');
      expect(MOCK_TEMPLATE_EXAMPLES).toContain('Example 3: Creating a mock factory');
    });
  });
});