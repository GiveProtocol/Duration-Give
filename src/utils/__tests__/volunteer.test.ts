...
import type { VolunteerApplication, VolunteerHours, VolunteerVerification } from '@/types/volunteer';
  ...
  supabase: {
    from: jest.fn(() => ({
      ...
      select: jest.fn(() => ({
        ...
        or: jest.fn(() => ({
          maybeSingle: jest.fn(() => ({ data: { id: 'verification-123' }, error: null }))
        }))
      }))
    }))
  }
...
describe('volunteer utils', () => {
  ...
  describe('createAcceptanceHash', () => {
    ...
    it('handles database errors gracefully', async () => {
      ...
      await expect(createAcceptanceHash(mockApplication)).rejects.toThrow('Failed to create acceptance hash');
    });
  });
  ...
  describe('verifyVolunteerHash', () => {
    ...
    it('returns false when database error occurs', async () => {
      ...
      mockSupabase.from = jest.fn(() => ({
        select: jest.fn(() => ({
          or: jest.fn(() => ({
            maybeSingle: jest.fn(() => ({ error: new Error('Database error') }))
          }))
        }))
      }));
