...
describe("Validation utilities", () => {
  ...
  describe("validateAuthInput", () => {
    it("should pass for valid email and password", () => {
      expect(() =>
        validateAuthInput("test@example.com", "ValidPass123!"),
      ).not.toThrow();
    });

    it("should throw error for invalid email", () => {
      expect(() => validateAuthInput("invalid-email", "ValidPass123!")).toThrow(
        "Please enter a valid email address",
      );
    });

    it("should throw error for invalid password", () => {
      expect(() => validateAuthInput("test@example.com", "weak")).toThrow(
  ...
  describe("sanitizeInput", () => {
    it("should remove dangerous characters", () => {
      ...
      expect(sanitizeInput("text with \"quotes\" and 'single quotes'")).toBe(
  ...
  describe("validateUrl", () => {
    it("should validate HTTPS URLs", () => {
      ...
      expect(validateUrl("https://subdomain.example.co.uk/path/to/page")).toBe(
        true,
      );
    });
