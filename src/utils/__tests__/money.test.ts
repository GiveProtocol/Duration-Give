import { EXCHANGE_RATES, CURRENCY_SYMBOLS, formatCurrency, calculateEquityGrowth } from '../money';

describe('money utilities', () => {
  describe('EXCHANGE_RATES', () => {
    it('contains expected currencies', () => {
      const expectedCurrencies = [
        'USD', 'CAD', 'EUR', 'CNY', 'JPY', 'KRW', 'AED', 'AUD', 
  ...
  describe('formatCurrency', () => {
    ...
    it('uses exchange rate 1 for unknown currency', () => {
      ...
      expect(() => formatCurrency(100, 'UNKNOWN')).toThrow('Invalid currency code');
    });
  ...
  describe('calculateEquityGrowth', () => {
    ...
    it('handles edge case amounts', () => {
      expect(calculateEquityGrowth(Number.MAX_SAFE_INTEGER)).toBe(Number.MAX_SAFE_INTEGER * 0.12);
      expect(calculateEquityGrowth(Number.MIN_SAFE_INTEGER)).toBe(Number.MIN_SAFE_INTEGER * 0.12);
    });
  });
