...
export const useVolunteerVerification = () => {
  ...
  const acceptApplication = async (applicationId: string) => {
    ...
    try {
      ...
      Logger.error("Application acceptance failed", {
        error: err,
        applicationId,
      });
      return null;
  ...
  const verifyHours = async (hoursId: string) => {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : "Failed to verify hours";
      setError(message);
      showToast("error", "Error", message);
  ...
  const getVerificationByHash = async (hash: string) => {
    try {
      ...
      const message =
        err instanceof Error ? err.message : "Failed to get verification";
      setError(message);
