...
jest.mock('@/components/ui/Button', () => ({
  Button: ({ children, onClick, variant, disabled }: { children: React.ReactNode; onClick?: () => void; variant?: string; disabled?: boolean }) => (
    <button onClick={onClick} className={variant} disabled={disabled}>{children}</button>
  ),
}));
jest.mock('@/components/contribution/DonationExportModal', () => ({
  DonationExportModal: ({ donations, onClose }: { donations: any[]; onClose: () => void }) => (
...
describe('GiveDashboard', () => {
  ...
  describe('Statistics Display', () => {
    it('displays total donations statistic', () => {
      ...
      expect(screen.getByText('dashboard.stats.totalDonations')).toBeInTheDocument();
    });

    it('displays volunteer hours statistic', () => {
      renderWithRouter(<GiveDashboard />);
      
      expect(screen.getByText('dashboard.stats.volunteerHours')).toBeInTheDocument();
    });

    it('displays impact score statistic', () => {
      renderWithRouter(<GiveDashboard />);
      
      expect(screen.getByText('dashboard.stats.impactScore')).toBeInTheDocument();
    });
  });
  ...
  describe('Data Display and Management', () => {
    it('displays transaction data in table format', () => {
      ...
      expect(screen.getByText('dashboard.table.organization')).toBeInTheDocument();
