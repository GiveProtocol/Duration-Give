...
describe("GiveDashboard", () => {
  ...
  describe("Authentication and Access Control", () => {
    ...
    it("renders dashboard when user is authenticated", () => {
      mockUseAuth.mockReturnValue(
        createMockAuth({ user: mockUser, userType: "donor" }),
      );
      renderWithRouter(<GiveDashboard />);
    ...
    it("shows loading state when auth is loading", () => {
      mockUseAuth.mockReturnValue(
        createMockAuth({ user: null, userType: null, loading: true }),
      );
      renderWithRouter(<GiveDashboard />);
  ...
  describe("Wallet Connection", () => {
    ...
    it("shows wallet address when connected", () => {
      mockUseWeb3.mockReturnValue(
        createMockWeb3({ address: "0x123...789", isConnected: true }),
      );
      renderWithRouter(<GiveDashboard />);
    ...
    it("handles wallet connection loading state", () => {
      mockUseWeb3.mockReturnValue(
        createMockWeb3({ address: null, isConnected: false, loading: true }),
      );
      renderWithRouter(<GiveDashboard />);
  ...
  describe("Filter and Sort Functionality", () => {
    ...
    it("renders status filter dropdown", () => {
      renderWithRouter(<GiveDashboard />);
      expect(
        screen.getByDisplayValue("dashboard.filters.allStatuses"),
      ).toBeInTheDocument();
    });
  ...
  describe("Statistics Display", () => {
    it("displays totalDonated statistic", () => {
      renderWithRouter(<GiveDashboard />);
      expect(
        screen.getByText("dashboard.stats.totalDonated"),
      ).toBeInTheDocument();
    });
    ...
    it("displays totalCharities statistic", () => {
      renderWithRouter(<GiveDashboard />);
      expect(
        screen.getByText("dashboard.stats.totalCharities"),
      ).toBeInTheDocument();
    });
    ...
    it("displays avgDonation statistic", () => {
      renderWithRouter(<GiveDashboard />);
      expect(
        screen.getByText("dashboard.stats.avgDonation"),
      ).toBeInTheDocument();
    });
    ...
    it("displays lastDonation statistic", () => {
      renderWithRouter(<GiveDashboard />);
      expect(
        screen.getByText("dashboard.stats.lastDonation"),
      ).toBeInTheDocument();
    });
  });
