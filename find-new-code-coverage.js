...
function getChangedFiles() {
  try {
    ...
    const gitDiff = execSync("git diff --name-only HEAD~5 HEAD", {
      encoding: "utf8",
    });
    const files = gitDiff
      .trim()
...
untestedFiles.forEach(({ file, lines }) => {
  const isChanged = changedFiles.some(
    (cf) => cf.includes(file) || file.includes(cf),
  );
...
console.log(
  `📊 Overall coverage estimate: ${((totalTestedLines / (totalTestedLines + totalUntestedLines)) * 100).toFixed(1)}%`,
);
