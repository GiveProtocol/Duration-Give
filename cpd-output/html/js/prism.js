...
var _self =
  ...
  Prism = (function (e) {
        ...
        highlightElement: function (a, n, s) {
          ...
          o &&
            ...
            (o.className =
              ...
              " language-" +
              i);
          var u = { element: a, language: i, grammar: l, code: a.textContent };
          function c(e) {
            ((u.highlightedCode = e),
          ...
          if (
            ...
            return (r.hooks.run("complete", u), void (s && s.call(u.element)));
      ...
      for (var d in a)
        if (a.hasOwnProperty(d) && a[d]) {
          ...
          for (var m = 0; m < p.length; ++m) {
            ...
            for (
              var k = h.pattern || h, x = n.next, w = o;
              ...
              if (!(P instanceof s)) {
                var A,
                  $ = 1;
                if (b) {
                  if (!(A = i(k, w, e, v))) break;
                  var S = A.index,
                    E = A.index + A[0].length,
                    _ = w;
                  for (_ += x.value.length; S >= _; )
                    _ += (x = x.next).value.length;
                  if (((w = _ -= x.value.length), x.value instanceof s))
                    continue;
                  for (
                    var j = x;
                    ...
                    ($++, (_ += j.value.length));
                  ($--, (P = e.slice(w, _)), (A.index -= w));
                } else if (!(A = i(k, 0, P, v))) continue;
                S = A.index;
                var C = A[0],
                  O = P.slice(0, S),
                  z = P.slice(S + C.length),
                  T = w + P.length;
                g && T > g.reach && (g.reach = T);
                var N = x.prev;
                if (
                  (O && ((N = u(t, N, O)), (w += O.length)),
                  c(t, N, $),
                  (x = u(t, N, new s(d, f ? r.tokenize(C, f) : C, y, C))),
                  z && u(t, x, z),
                  $ > 1)
                ) {
                  var L = { cause: d + "," + m, reach: T };
                  (l(e, t, a, x.prev, w, L),
    ...
    if (
      ...
      "loading" === p || ("interactive" === p && g && g.defer)
